...
if __name__ == "__main__":
    ...
    open_time = datetime.strptime("08:00:00", "%H:%M:%S").time()
    close_time = datetime.strptime("22:00:00", "%H:%M:%S").time()
    tea_time = datetime.strptime("15:00:00", "%H:%M:%S").time()
    Time_multiplier = 1
    if open_time <= virtual_time < tea_time:
        Time_multiplier = 1.2
    elif tea_time <= virtual_time < close_time: 
        Time_multiplier = 1.5
    print(Time_multiplier)
    print(clock_data["virtual_time"].time())
    if clock_data["virtual_time"].time() < open_time:
        print("茶馆尚未营业")
    else:
        print("茶馆正在营业中")
